...
void HandGesture::checkForOneFinger(Mat src){
	...
	int yTol = bRect.height / 6; // gia tri cong them voa de xem có phải trương hợp 1 ngón tay hay là trường ko ko co ngón nào
	Point highestP;
	highestP.y = src.rows;
	...
	// xet truong hop khong co ngon tay nao
	int n = 0;
	d = hullP.begin();
	while (d != hullP.end()) {
		Point v = (*d);
