...
Mat returnSubBackgroundStatic(Mat frameCurrent, Mat framePrev, Rect box){
	...
	imshow("background substraction", anhGrab);
	//////////////////////////////////////////////////////////////////////////////////////////////
...
Mat returnSubBackgroundDynamic(Ptr<BackgroundSubtractor> bg_model, Mat img){
	...
	bg_model->apply(gray, foregroundMask, true ? -1 : 0);
...
void drawHist(String name, Mat src){
	...
	int hist_w = 512; int hist_h = 400;// kich thước cua so cua man hinh hien thi histogram
...
#ifdef __linux__
	bbox.x = Point(m->src.cols / 4 - 40, m->src.rows / 2).x;
	bbox.y = Point(m->src.cols / 3 - 40, m->src.rows / 6).y;
	bbox.width = std::abs(Point(m->src.cols / 2 - 60 + square_len, m->src.rows / 1.5 + square_len).x - Point(m->src.cols / 4 - 40, m->src.rows / 2).x);
	bbox.height = std::abs(Point(m->src.cols / 2 - 60 + square_len, m->src.rows / 1.5 + square_len).y - Point(m->src.cols / 3 - 40, m->src.rows / 6).y);
#endif
...
void analyContour(Mat src, Shape *sh, int idxSh, HandGesture *hg, Rect boxCheckHand){
	...
	if (isHand && hand){
		...
		rectangle(src, bRectSh.tl(), bRectSh.br(), Scalar(0, 0, 200),3);
	}
}
...
void makeContours(Mat src, Mat bw, Shape * sh, HandGesture *hg1,Rect boxCheckHand){
	...
	cv::dilate(aBw, aBw, getStructuringElement(MORPH_ELLIPSE, Size(5, 5)));
	cv::erode(aBw, aBw, getStructuringElement(MORPH_ELLIPSE, Size(5, 5)));
	//Canny(aBw, aBw, threshold_canny, threshold_canny*2.5, 3);
...
void handDect(Mat &m){
	...
	vector<Rect> palm;
	Palm.load("myhaar.xml");
	Palm.detectMultiScale(m1, palm, 1.1, 3, 0 | CV_HAAR_FIND_BIGGEST_OBJECT, Size(3, 3));
	Point sh;
	if (palm.size()){
		rectangle(m, palm[0], Scalar(216, 220, 15), CV_FILLED);
	}
}
...
Rect camShiftDemo(Mat &src, Mat &imageHLS, Mat& maskHLS, Mat &hue, Mat &hist, const float * phranges, int &hsize, Rect &trackWindow, bool &flagCam){
	...
	if (flagCam){
		...
		//uniform tuc la qui bin deu hay la khong dong deu. phranges la mang chua mang 2 gia tri L va U trong la bins.
	...
	//TermCriteria la lop xac dinh tieu chuan dừng cho thuật toán lặp lại
	//eps: độ chính xác mong muốn hoặc thay đổi thông số mà thuật toán lặp lại dừng
	//argument thu 2 maxcount: Số lần lặp hoặc yếu tố tối đa để tính toán
	//argument so 3: eps: độ chính xác mong muốn hoặc thay đổi thông số mà thuật toán lặp lại dừng
...
Point adjustCusor(Point cursor, KalmanFilter &KF, Mat_<float> &measurement){
		// First predict, to update the internal statePre variable
		KF.predict();
		measurement(0) = cursor.x;
		measurement(1) = cursor.y;
		// The update phase 
...
float returnPercentDiff(Mat src_base, Mat src_test1){
	...
	per = compareHist(hist_base, hist_test1, CV_COMP_CORREL);
	return per;
}
	...
	while (1){
		...
		while (1){
			...
			if (flag1){
				...
				int timed = clock();
				times = timed - times;
				cout << "time backgroundsub : " << times << endl;
			...
			int timed_m = clock();
			times_m = timed_m - times_m;
			cout << "time main : " << times_m << endl;
...
#else
				Point newC = adjustCusor(hg1.getPointCenter(),KF,measurement);
...
//	for (int i = 0; i < 3; ++i)
//	{
//		data_pts.at<double>(i, 0) = a[i][0];
//		data_pts.at<double>(i, 1) = a[i][1];
//	}
//	//Perform PCA analysis
//	PCA pca_analysis(data_pts, Mat(), CV_PCA_DATA_AS_ROW);
//
...
//	Point pos = Point(pca_analysis.mean.at<double>(0, 0),
//		pca_analysis.mean.at<double>(0, 1));
//	cout << "mean " << pca_analysis.mean.at<double>(0, 1) << endl;
//
//	//Store the eigenvalues and eigenvectors
//	vector<Point2d> eigen_vecs(2);
//	vector<double> eigen_val(2);
...
//		eigen_val[i] = pca_analysis.eigenvalues.at<double>(i, 0);
//	}
//	cout << " " << pca_analysis.mean.at<double>(0, 0) << " " << pca_analysis.mean.at<double>(0, 1)<<endl;
//
//	cout << " " << eigen_val[0] << " " << eigen_val[1] << endl;
//	cout << " " << eigen_vecs[0] << " " << eigen_vecs[1] << endl;
//
//	waitKey(0);
//	return 0;
//}